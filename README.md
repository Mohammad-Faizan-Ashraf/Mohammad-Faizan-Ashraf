...
#### 🏢 I.T. Solutions 4 India | Delhi | Feb 2024 – Jan 2025
...
| ---------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `ZoneValue (Real Estate Platform)` | Multi-role B2B real estate platform backend featuring property listings, user-based lead routing, RBAC-secured access layers, and automated sales pipelines with Django, DRF, and PostgreSQL.                  |
| `LMS Backend`                      | Institutional-grade LMS backend with modular user roles (admin/instructor/student), multi-tenant architecture, dynamic course lifecycle management, and real-time result tracking. Fully DRF-driven API stack. |
| `Campaign Messaging & ERP`         | High-throughput campaign automation backend with WhatsApp Cloud API, webhook status feedback, and ERP sync modules. Features async processing, webhook event engines, and campaign analytics.                  |
| `AuthRabbit`                       | Centralized access control system with admin dashboards, token lifecycle governance, custom permissions, session policy enforcement, and multi-tenant access visibility. Powered by JWT + RBAC.                |
| `Internal CRM`                     | CRM backend engine designed for internal operations—inventory handling, task automation, lead capture, and audit trails. Supports Celery-based background task queues and structured permission workflows.     |
| `API Documenter`                   | Internal tool to auto-generate OpenAPI-compliant interactive API documentation from DRF schema definitions, reducing integration time for third-party teams.                                                   |
| `AssignPad`                        | Education-focused assignment lifecycle platform with role-based submission workflows, instructor moderation, class-level controls, and deadline policy automation.                                             |
| `Network Monitoring System`        | Lightweight backend module for collecting infrastructure metrics from distributed endpoints, streaming anomaly alerts, and rendering live dashboards for NOC teams.                                            |
