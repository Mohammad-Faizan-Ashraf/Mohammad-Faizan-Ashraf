...
| `AuthRabbit Recognition Suite`                       | Centralized access control system with admin dashboards, token lifecycle governance, custom permissions, session policy enforcement, and multi-tenant access visibility. Powered by JWT + RBAC.                |
...
| `Custom Request DocTest`                   | Internal tool to auto-generate OpenAPI-compliant interactive API documentation from DRF schema definitions, reducing integration time for third-party teams.                                                   |
| `AssignPad Console`                        | Education-focused assignment lifecycle platform with role-based submission workflows, instructor moderation, class-level controls, and deadline policy automation.                                             |
